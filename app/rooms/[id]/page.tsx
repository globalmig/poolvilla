  ...
  return (
    <main>
      ...
      <section className="relative" style={{ height: "75vh" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-10 text-white pointer-events-none">
          <div className="max-w-7xl mx-auto">
            <span className="inline-flex items-center rounded-full bg-white/15 backdrop-blur-sm px-4 py-1.5 text-sm font-semibold mb-3 text-white/90">{room.type}</span>
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight">{room.name}</h1>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-8">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16">
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-5">Facilities</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {room.features.map((f) => (
                <div key={f} className="rounded-2xl bg-[#FAFAF9] px-5 py-4 text-sm text-gray-700 font-medium flex items-center gap-2.5">
            ...
            {(room.pool || room.bbq || room.equipment || room.extraGuestFee || room.schedule) && (
              <div className="mt-12 space-y-8">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">이용안내</p>

                {room.extraGuestFee && (
                  <div>
                    <p className="text-sm text-gray-800 font-semibold mb-1.5">인원추가요금</p>
                    <p className="text-sm text-gray-500 leading-relaxed">{room.extraGuestFee}</p>
                  </div>
                )}
                ...
                {room.bbq && (
                  <div>
                    ...
                    <p className="text-sm text-gray-500 leading-relaxed">
                      {room.bbq.desc} · <s className="text-red-500">{room.bbq.fee}</s> <span className="text-red-600 font-semibold">{PROMO_LABEL}</span>
                    </p>
                  </div>
                )}
                ...
                {room.equipment && (
                  <div>
                    ...
                    <p className="text-sm text-gray-500 leading-relaxed">{room.equipment.join(", ")}</p>
                  </div>
                )}
          ...
          <div>
            <div className="rounded-3xl bg-[#FAFAF9] p-8 sticky top-24 shadow-sm">
              <p className="text-xs font-semibold text-[#2A8EA2] uppercase tracking-wide mb-1">{room.type}</p>
              ...
              <Link href="/booking" className="btn-pop block w-full text-center py-3.5 rounded-full bg-[#2A8EA2] text-white text-sm font-semibold shadow-sm hover:shadow-lg transition-shadow mb-3">
                실시간예약
              </Link>
              <Link
                href="/reservation"
      ...
      <section className="pb-16 px-8">
        <div className="max-w-7xl mx-auto">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-5">Photos</p>
          <div className="fill-last-row-2-3-4-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {images.map((src, i) => (
              <div key={i} className="relative overflow-hidden rounded-2xl aspect-square">
                <Image src={src} alt={`${room.name} ${i + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-500" unoptimized />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-8 pb-10">
        <Link href="/rooms" className="text-sm font-semibold text-gray-500 hover:text-gray-900 transition-colors">
          ← 객실 목록으로
        </Link>
      </div>
