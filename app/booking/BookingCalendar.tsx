...
import {
  ...
  FiUser, FiPhone, FiMail, FiMessageSquare, FiUsers,
  FiCheckCircle, FiAlertCircle, FiCheck,
} from 'react-icons/fi';
...
function isPeak(dateStr: string):   boolean { return PEAK_MD.has(dateStr.slice(5)); }
function isWeekend(dateStr: string): boolean { const d = new Date(dateStr).getDay(); return d === 5 || d === 6; }
...
function calcTotal(roomType: RoomType, checkIn: Date, checkOut: Date): number {
  ...
  while (cur < checkOut) { sum += nightlyRate(roomType, fmt(cur)); cur.setDate(cur.getDate() + 1); }
  return sum;
}
...
function sameDay(a: Date, b: Date) { return fmt(a) === fmt(b); }
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors
                  ${done   ? 'bg-[#2A8EA2] text-white' :
                    active ? 'bg-[#2A8EA2] text-white ring-4 ring-[#2A8EA2]/20' :
...
export default function BookingCalendar() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setApiError(data.error ?? '오류가 발생했습니다.'); return; }
      setStep('success');
    } catch {
  ...
  function reset() {
    setCheckIn(null); setCheckOut(null); setSelectedRoom(null);
    setStep('room');
        ...
        <div className="rounded-3xl bg-[#FAFAF9] p-6 text-left mb-8 text-sm text-gray-600 space-y-1.5">
          ...
          {selectedExtras.size > 0 && (
            <div>
              ...
              {EXTRAS.filter(e => selectedExtras.has(e.id)).map(e => e.label).join(', ')}
            </div>
          )}
          <div className="pt-1 border-t border-gray-200">
            <span className="font-semibold text-gray-800">총 금액:</span>{' '}
            <span className="font-bold text-[#2A8EA2]">{krw(total)}</span>
          </div>
        </div>
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-10">
              ...
              <div key={type}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {rooms.map(roomId => (
                    <button
                      key={roomId}
                      onClick={() => chooseRoom(roomId)}
                      className="btn-pop rounded-2xl border-2 border-gray-100 hover:border-[#2A8EA2] px-4 py-4 text-left transition-colors"
                    >
        ...
        <p className="text-xs text-gray-400 text-center mt-8">
          객실을 선택하면 해당 객실의 예약 가능한 날짜를 확인할 수 있습니다.
        </p>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiUser className="inline mr-1.5" />이름 <span className="text-red-500">*</span>
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiPhone className="inline mr-1.5" />연락처 <span className="text-red-500">*</span>
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiMail className="inline mr-1.5" />이메일 <span className="text-gray-400 font-normal">(선택)</span>
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiUsers className="inline mr-1.5" />인원 <span className="text-red-500">*</span>
            </label>
            ...
            <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">
              기준 인원 초과 시 1인당 30,000원/박 추가 · 성인·아동·유아·영유아 모두 인원수에 포함 · 최대 인원 초과 시 펜션으로 문의
            </p>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left rounded-2xl border-2 px-5 py-4 transition-all ${
                    on ? 'border-[#2A8EA2] bg-[#2A8EA2]/5' : 'border-gray-100 hover:border-gray-300'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <p className={`text-sm font-semibold ${on ? 'text-[#2A8EA2]' : 'text-gray-800'}`}>
                        {extra.label}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5">{extra.desc}</p>
                    </div>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-colors ${
                      on ? 'bg-[#2A8EA2]' : 'bg-gray-100'
                    }`}>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiMessageSquare className="inline mr-1.5" />요청사항 <span className="text-gray-400 font-normal">(선택)</span>
            </label>
            <textarea rows={3} placeholder="특별한 요청사항이 있으시면 입력해주세요."
              value={form.notes}
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            예약 신청 후 예약금(총 금액의 30%) 입금 확인 시 예약이 확정됩니다.
          </p>

          {/* ── 약관 동의 ── */}
          <div className={`flex items-center justify-between rounded-2xl px-5 py-4 border transition-colors ${
            agreedToTerms ? 'bg-[#2A8EA2]/5 border-[#2A8EA2]/20' : 'bg-gray-50 border-gray-100'
          }`}>
            <div>
              <p className="text-sm font-medium text-gray-800">개인정보 수집·이용 및 예약 약관</p>
              <p className={`text-xs mt-0.5 ${agreedToTerms ? 'text-[#2A8EA2]' : 'text-gray-400'}`}>
                {agreedToTerms ? '동의 완료' : '예약 진행을 위해 동의가 필요합니다'}
              </p>
            </div>
            <button
              type="button"
              onClick={() => setShowTermsModal(true)}
              className={`text-sm font-semibold px-4 py-2 rounded-full transition-colors shrink-0 ${
                agreedToTerms
                  ? 'bg-[#2A8EA2] text-white'
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-3xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl">
              <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                ...
                <button type="button" onClick={() => setShowTermsModal(false)} className="text-gray-400 hover:text-gray-700 text-lg leading-none">✕</button>
              </div>
              ...
              <div className="overflow-y-auto px-6 py-5 space-y-6 text-sm text-gray-600 leading-relaxed">
                ...
                <section>
                  <h3 className="font-bold text-gray-800 mb-2">2. 예약금 및 결제</h3>
                  <ul className="space-y-1.5 text-gray-500">
                    <li>· 예약금(총 금액의 30%) 입금 확인 후 예약이 확정됩니다.</li>
                    <li>· 잔금은 체크인 당일 현장 결제 또는 사전 계좌이체로 납부해 주세요.</li>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-10">
        ...
        <div>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200">
              {days.map((d, i) => {
                ...
                else if (isEnd)               { bg = 'bg-[#2A8EA2]'; textCls = 'text-white font-bold cursor-pointer'; }
                else if (ranged)              { bg = 'bg-[#2A8EA2]/10'; textCls = 'text-gray-800 cursor-pointer'; }
                ...
                return (
                  ...
                  >
                    <span>{d.getDate()}</span>
                    {pk && s !== 'past' && s !== 'unavailable' && (
                      <span className="absolute top-1 right-1 text-[7px] text-[#2A8EA2] font-bold leading-none">성</span>
                    )}
                    {!pk && wk && s !== 'past' && s !== 'unavailable' && (
                      <span className="absolute top-1 right-1 text-[7px] text-blue-300 font-bold leading-none">주</span>
                    )}
                  </div>
                );
              })}
          ...
          <div className="flex flex-wrap gap-4 mt-6 pt-5 border-t border-gray-100 text-xs text-gray-500">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-[#2A8EA2] inline-block rounded-full" />선택</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-white border border-gray-300 inline-block rounded-full" />예약 가능</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-gray-100 inline-block rounded-full" />예약 불가</span>
            <span className="flex items-center gap-1.5 ml-auto"><span className="text-[#2A8EA2] font-bold text-[9px]">성</span>성수기</span>
            <span className="flex items-center gap-1.5"><span className="text-blue-300 font-bold text-[9px]">주</span>주말</span>
          </div>

          {/* Hint */}
          <p className="mt-4 text-sm text-gray-400">
            {!checkIn ? '체크인 날짜를 선택하세요.'
             : !checkOut ? '체크아웃 날짜를 선택하세요.'
             : `${fmtKR(fmt(checkIn))} → ${fmtKR(fmt(checkOut))}  (${nights}박)`}
          </p>
        </div>
          ...
          {checkIn && checkOut && (
            <div className="rounded-3xl bg-[#1C2B3A] p-5 text-white shadow-sm">
              ...
              <button
                ...
                className="btn-pop w-full py-3 rounded-full bg-[#2A8EA2] text-white font-bold shadow-sm hover:shadow-lg transition-shadow"
              >
                예약 신청하기
              </button>
            </div>
